...
The release zip includes a self-contained Core under `mnemonic_hook/core/`. You do not install .NET or FFmpeg separately.

## Launching Core

Core starts from the Hook dock:

1. **Start recording** (recommended) — Hook launches bundled Core automatically. The dock shows **Core started (bundled)**.
2. **Auto-launch** — Optional Hook setting to start Core when Godot opens.

To use a different executable, set **Editor → Editor Settings → mnemonic_hook → Core Windows Exe**. See [Installation](../installation.md#custom-core-path).
...
| **Segment log…** | Full segment history (no search — use Hook dock for that) |
...
Tray **Settings…** owns audio device selection and capture restart. Capture retention and heuristic toggles live in the Hook dock **Capture…** panel — both read/write the same `settings.json`.

## What Core does each segment

1. **Capture** screen and audio into the current scratch buffer
2. **Tail-poll** session events from Hook
